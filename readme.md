...
- ✅ **思考动作（推理指示）**：推理模型（如 `deepseek-flash`）流式返回 `reasoning_content`，这段时间**立绘切到 `Think`**，正文一开始流就切成 `Talk`——看一眼动作就知道模型在想还是在说。文字输入不再改状态（保持原样）。**推理期间不随机移动**，免得思考动作被 `Move` 素材盖住。`Think` 缺素材时回落到 `Idle`，不会僵在 `Default` 上。
