...
│   │   ├── clock.py                 # 时间文本化（时间旁白与将来的取时间工具共用）
...
- **L1 工作上下文**：每次请求组装 `基础提示词 + 角色提示词 + 长期记忆 + 相关记忆 + 摘要 + 最近 N 轮`，N 由 `max_context_messages` 决定；必要时在当轮消息前加一条**时间旁白**（见下）。
- **时间旁白**：模型需要"对表"时，在**最后一条用户消息前**加一行 `[旁白] 现在是 2026-09-18（周五）20:50`（`clock.now_text()` 生成）。**不放进 system**：常驻内容会被模型当成当前任务，于是反复提起时间；而且程序可能挂机很久，system 里写死的时间会过时。触发条件是**本会话第一轮**，或距上一条用户消息超过 `NARRATION_GAP_MINUTES`（30 分钟）——同一段对话里连着聊时不提。**只给"现在"，不给"距上次多久"**：间隔让模型自己从记忆的日期去推，否则它容易开口就"三天前你说过……"。旁白与 `[被打断]` 一样**只进请求**，落盘与界面都看不到；上下文里真的带旁白时，才会注入一小段说明告诉模型这**不是用户说的话、不要复述**。
