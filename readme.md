...
- ✅ 设置面板（900×740，无边框四页）：
  ...
  - **工具**：番茄钟时长、天气城市；
  - **开发者**：启用开发者面板（开关 + 打开按钮）。
- ✅ **开发者面板**：在「设置 → 开发者」里开启后弹出独立窗口，上半是**对话链路**、下半是**日志**。能看到每次请求拼出的完整上下文（系统段逐段列出、对话背景、每一轮消息的角色与时间）、实发条数与总字数、模型与温度、首字延迟与总耗时、是否被打断，以及发请求 / 打断 / 失败 / 记忆整理等运行日志。**记录只在内存里**（上限 500 条），关掉开关即清空，不写文件也不进版本控制。
...
│   │   ├── clock.py                 # 时间文本化（对话背景与将来的取时间工具共用）
...
- **L1 工作上下文**：每次请求组装 `基础提示词 + 角色提示词 + 标记说明 + 长期记忆 + 相关记忆 + 摘要 + 最近 N 轮`，N 由 `max_context_messages` 决定；必要时在当轮消息前加一条**对话背景**（见下）。
- **对话背景（时间）**：模型需要"对表"时，在**最后一条用户消息前**加一行 `[对话背景] 现在是 2026-09-19（周六）17:56`（`clock.now_text()` 生成，标记名是 `message.NARRATION_TAG`）。**不放进 system**：常驻内容会被模型当成当前任务，于是反复提起时间；而且程序可能挂机很久，system 里写死的时间会过时。触发条件是**本会话第一轮**，或距上一条用户消息超过 `NARRATION_GAP_MINUTES`（30 分钟）——同一段对话里连着聊时不提。**只给"现在"，不给"距上次多久"**：间隔让模型自己从记忆的日期去推，否则它容易开口就"三天前你说过……"。这一行与 `[被打断]` 一样**只进请求**，落盘与界面都看不到；本次真的带上了它，才会在 `【标记说明】` 里多一句说明告诉模型这**不是用户说的话**。
- **记忆的时间**：召回注入时给**事件与约定**补上发生日期（`- （事件，9月12日）用户下周要去上海出差三天`），事实与偏好是长期属性、不带时间。整理记忆时会把会话的日期（单日连星期）作为锚点喂给抽取提示词，并要求把"明天""下周"这类说法**换算成绝对日期**再写进 `text`——没有这个锚点，相对时间会被原样存下来，过两周就成了悬空记忆。
...
- **背景资料标注**：长期记忆、相关记忆、摘要三段都带一句 `（背景资料，只在相关时自然带出，不要主动复述）`。常驻内容不声明用途时，模型容易把它们当成当前任务，反复主动提起。
- **system 的分块标题**：拼进 system 的每一块都带一个大标题，模型据此分辨每段是什么、该按什么权重对待——`【基本原则】`（全局基础提示词）/ `【角色设定】`（角色提示词）/ `【标记说明】`（表情标记 + 对话背景的说明合成一节）/ `【被打断】` / `【长期记忆】` / `【相关记忆】` / `【过往摘要】`。标题集中在 `app/conversation/prompts.py`，**改文案只改那一个文件**；拼装发生在 `service.py` 的 `_system_prompt()`（原则、人设）与 `_runtime_notes()`（表情说明，交给下一层）以及 `memory.py` 的 `build_context()`（`【标记说明】` 及以下）。用 `【】` 而不是 Markdown 的 `###`：基础提示词要求模型只输出纯文本，避免它把标题风格学进回复。
